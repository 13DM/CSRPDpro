...
if %errorlevel% == 0 goto QuickDropMain
...
if [%1]==[nsb] (
	"%MYFILES%\CSRPacker.exe" unpack -i %1 -p >nul
	exit /b
)
if [%1]==[nsb.txt] (
	"%MYFILES%\CSRPacker.exe" pack -i %1 -m -o nsb >nul
	exit /b
)
if [%1]==[scb] (
	"%MYFILES%\CSRPacker.exe" unpack -i %1 -p >nul
...
if [%1]==[scb.txt] (
	"%MYFILES%\CSRPacker.exe" pack -i %1 -m -o scb >nul
	exit /b
)
if [%1]==[trb] (
	"%MYFILES%\CSRPacker.exe" unpack -i %1 -p >nul
...
if [%1]==[trb.txt] (
	"%MYFILES%\CSRPacker.exe" pack -i %1 -m -o trb >nul
	exit/b
)

cls
Echo File not compatable for packing or unpacking.
echo.
echo Compatable files for quick drop are:
echo.
echo Unpack: nsb , scb, trb
echo - - - - - - - - - - - - - - - - - - - - - - -
echo Pack: nsb.txt, scb.txt, trb.txt 
echo.
timeout 5 >nul
exit /b
...
Call :MainLogo
echo     (1.) NSB Menu -- Unpack and edit nsbs                                          [ Deluxe Options Below]
echo.  
echo     (2.) SCB Menu -- Unpack and edit scbs                                          (e.) ESNN Builder  
echo.  
echo     (3.) TRB Menu -- Unpack and edit trbs                                          (s.) Support Builder
echo.
echo     (4.) TRBgen -- Tool to generate trb files to add RP                            (a.) Car Name List
echo.
...
CHOICE /C 12345678es /M "Pick an operation." >nul

::Error Levels are in decreasing order

IF ERRORLEVEL a GOTO CarListOpen
IF ERRORLEVEL s GOTO SupportBuilder
IF ERRORLEVEL e GOTO ESNN
...
if ERRORLEVEL 1 set trbmanutime=1
...
REM (%yearmod%*31536000)+(%leapmod%*86400)+((%tmstg%+%day%-1)*86400)+((%utchour%-1)*3600)+(%mins%*60)
...
rem set /a smts=(%yearmod%*31536000)+(%leapmod%*86400)+((%tmstg%+%day%-1)*86400)+((%utchour%-1)*3600)+(%mins%*60)+%secs%
...
echo     Time stamp set of: %smts% at %date% %time% >>"%USERPROFILE%\%Documents%\CSRPpro\CSRPproShellInstance"
...
echo   / //_ ^<  / // / /^|_/ /   current utc time: %day%/%mon%/%year% %utchour%:%displayutcmin%:%displaysecs% 
echo  /_/____/ /____/_/  /_/    last car type made: %carname%
echo ------------------------   last gacha config: %varianttype%
echo ---------------------------------------------------------------------
...
echo       }>>supmetha.txt
echo.
echo     Added %numberof% RP to build at %date% %time% >>"%USERPROFILE%\%Documents%\CSRPpro\CSRPproShellInstance"
   choice /C YN /M "Do you want to add more to the file?"
   if ERRORLEVEL 2 (
       echo     }>>supmetha.txt
       goto SUPMETHFINISHBUILD
...
echo  HHHHHHHH                 HHHHHHHHHHHHHHH     HHHHHHHHH  HHHHHHHHHHHHH( HHHHHHHHH  HHHHHHHHHHHHHH   
echo  HHHHHHHH                   HHHHHHHHHHHHHH   HHHHHHHHH  HHHHHHHHHHHHH   HHHHHHHHH HHHHHHHHHHHHH     v 1.0.0.6
...
REM -------------------------------------------------------------------------------------------------------------------------------------
REM -------------------------------------------------------------------------------------------------------------------------------------
REM                                           START OF THE EPOCH TIME STAMP GET
REM -------------------------------------------------------------------------------------------------------------------------------------
REM -------------------------------------------------------------------------------------------------------------------------------------
